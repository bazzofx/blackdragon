...
# ─── Data Collection (Scan Runners) ──────────────────────────────────────────

def check_tool(name):
    """Verify a scanning tool is available on the system PATH."""
    try:
...
def resolve_to_ip(domain):
    """
    Resolve a domain name to its IPv4 address via nslookup.
    Falls back to Python's socket.getaddrinfo.
    Returns the IP string or None on failure.
    """
    ip = None

    # Try nslookup first (more reliable on some networks)
    try:
        ...
        for line in result.stdout.splitlines():
            line = line.strip()
            # Match lines like: "Address:  192.0.2.1" or "Address: 192.0.2.1"
            if line.lower().startswith("address:"):
                parts = line.split()
                for p in parts:
                    if ":" not in p and p.replace(".", "").isdigit():
                        ip = p
                        break
                if ip:
                    break
    ...
    # Fallback: Python socket resolution (with 5s timeout via threading)
    if not ip:
        import threading
        ...
        def _resolve():
            try:
                _result[0] = socket.getaddrinfo(domain, 80, socket.AF_INET)[0][4][0]
...
def run_whatweb(target):
    """
    Run WhatWeb fingerprinting against a domain/FQDN.
    Outputs JSON to whatWebResult.json and verbose log to whatweb_results.log.
    """
    if not check_tool("whatweb"):
        print("    ⚠ 'whatweb' not found on PATH — skipping WhatWeb scan")
        return False

    json_path = os.path.join(BASE_DIR, "whatWebResult.json")
    log_path = os.path.join(BASE_DIR, "whatweb_results.log")
    ...
    try:
        ...
        if os.path.exists(json_path):
            print(f"    → WhatWeb complete → {json_path}")
            return True
        else:
            print(f"    ⚠ WhatWeb did not produce output. stderr:\n{result.stderr[:500]}")
            return False
    except subprocess.TimeoutExpired:
        print("    ⚠ WhatWeb timed out (600s)")
        return False
...
def run_nmap(ip):
    """
    Run Nmap full port scan against an IP address.
    Outputs to nmap_scan.xml / .nmap / .gnmap.
    Expects 'ip' to be a raw IPv4 address.
    """
    if not check_tool("nmap"):
        print("    ⚠ 'nmap' not found on PATH — skipping Nmap scan")
        return False

    out_prefix = os.path.join(BASE_DIR, "nmap_scan")

    print(f"[*] Running Nmap full scan against {ip} (this may take a while) ...")
    cmd = [
        "nmap",
        "-sS", "-sV", "-O", "-p-", "-T4", "-A",
        "-oA", out_prefix,
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=3600,  # up to 1 hour
        )
        xml_path = f"{out_prefix}.xml"
        if os.path.exists(xml_path):
            print(f"    → Nmap complete → {xml_path}")
            # Show a brief summary of open ports
            for line in result.stdout.splitlines():
                if "/tcp" in line and "open" in line:
                    print(f"      {line.strip()}")
            return True
        else:
            print(f"    ⚠ Nmap did not produce XML output. stderr:\n{result.stderr[:500]}")
            return False
    except subprocess.TimeoutExpired:
        print("    ⚠ Nmap timed out (3600s) — partial results may exist")
        return False
    except Exception as e:
...
def run_dirsearch(target):
    """
    Run Dirsearch directory enumeration against a URL (http:// or https://).
    Outputs JSON to dirsearch_results.json.
    """
    if not check_tool("dirsearch"):
        print("    ⚠ 'dirsearch' not found on PATH — skipping Dirsearch scan")
        return False

    json_path = os.path.join(BASE_DIR, "dirsearch_results.json")
    ...
    print(f"[*] Running Dirsearch against {url} ...")
    cmd = [
        "dirsearch",
        "-u", url,
        "-e", "php,asp,aspx,jsp,html,js,txt,bak,old,zip,sql,conf,config,env,log,swp",
        "--random-agent",
        "-t", "30",
        "-x", "429,500,503",
        "--format=json",
    ...
    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=1800,  # up to 30 min
        )
        if os.path.exists(json_path):
            print(f"    → Dirsearch complete → {json_path}")
            return True
        else:
            print(f"    ⚠ Dirsearch did not produce JSON output. stderr:\n{result.stderr[:500]}")
            return False
    except subprocess.TimeoutExpired:
        print("    ⚠ Dirsearch timed out (1800s)")
        return False
    except Exception as e:
...
def run_data_collection(target):
    """
    Orchestrate all three scans (WhatWeb, Nmap via nslookup, Dirsearch)
    against the given target domain/FQDN.
    Returns True if at least one scan succeeded.
    """
    if not target:
        print("[-] No target provided — skipping data collection")
        return False

    print(f"\n{'═' * 50}")
    print(f"  Data Collection Phase — Target: {target}")
    print(f"{'═' * 50}\n")

    any_success = False

    # 1. WhatWeb (uses domain directly)
    if run_whatweb(target):
        any_success = True

    # 2. Nmap (needs IP — resolve first)
    print(f"[*] Resolving {target} to IPv4 ...")
    ...
    if ip:
        ...
        print(f"    ⚠ Could not resolve {target} to an IP — skipping Nmap scan")

    # 3. Dirsearch (uses URL)
    if run_dirsearch(target):
        any_success = True
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Cyber Samurai — Fingerprint & Security Assessment Report Generator",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  %(prog)s --scan example.com        # Scan target + generate report\n"
            "  %(prog)s --scan-only example.com   # Only collect data, skip report\n"
            "  %(prog)s                           # Parse existing data files only\n"
        ),
    )
    parser.add_argument(
        "--scan", metavar="TARGET",
        help="Run full data collection (WhatWeb + Nmap + Dirsearch) against TARGET "
             "then generate the report. TARGET can be a domain (example.com) or FQDN.",
    )
    parser.add_argument(
        "--scan-only", metavar="TARGET",
        help="Run data collection only — skip report generation. "
             "Useful when you want to review raw scan output first.",
    )
    return parser.parse_args()


def main():
    """Main entry point: optionally run scans, parse data sources, generate report."""
    args = parse_args()
    ...
    # ── Determine mode ──
    collect_target = args.scan or args.scan_only
    generate_report = bool(args.scan)  # --scan generates report; --scan-only doesn't

    if collect_target:
        # ── Step 0: Run data collection ──
        run_data_collection(collect_target)

        if args.scan_only:
            print("[*] --scan-only mode: data collection complete. Skipping report generation.")
            print()
            return

    else:
        print("[*] No --scan target provided — using existing data files.")
        print()
